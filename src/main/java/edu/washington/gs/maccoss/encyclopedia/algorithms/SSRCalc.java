...
/**
 * User: tholzman Date: Jul 13, 2006 Time: 9:43:40 AM
 */
/* Translated from perl to C, Ted Holzman FHCRC, 6/2006 */
/* Retranslated from C to Java, Ted Holzman FHCRC 7/2006 */
/*
 * NB: This is a version 0.1 direct translation. /* An attempt has been made to
 * keep function names, variable names, and algorithms /* as close as possible
 * to the original perl. /* /* A real translation into native java idiom would
...
public class SSRCalc {
	 ...
	 */
	/*
	 * translation of the ReadParmFile perl routine. This does not read a
	 * parameter
	 */
	...
	private static class AAParams {
		...
		AAParams(char aa, double rc, double rc1, double rc2, double rcn, double rcn2, double rcs, double rc1s, double rc2s, double rcns, double rcn2s, double undkrh, double amass, double ct,
				double nt, double pk, double h2bascore, double h2cmult) {
			RC=rc;
			RC1=rc1;
			RC2=rc2;
			RCN=rcn;
	...
	/*
	 * Translator's note: For the Java version we are prepending and appending
	 * 0s to the "pick" (key) column. This is done dynamically and repeatedly in
	 * the perl code. As far as I can tell, pick is never used without the
	 * surrounding 0s.
	 */
	...
	// that the constant data in the static initialization blocks has been
	// "tuned"
	...
	private static final double Z01=-0.03, Z02=0.60, NDELTAWT=0.8; // negative
																	// delta
																	// values
	private static final double Z03=0.00, Z04=0.00, PDELTAWT=1.0; // positive
																	// delta
																	// values
	...
	public static double getHydrophobicity(String peptide) {
		double tsum3=0.0;
		int i;
		int sze;
		...
		sze=peptide.length();
		if (sze<4)
			return tsum3; // peptide is too short ot have any retention
		if (sze<10) { // short peptides use short peptide retention weights
			tsum3=AAPARAMS.get(peptide.charAt(0)).RC1S+ // Sum weights for 1st
					AAPARAMS.get(peptide.charAt(1)).RC2S+ // second,
					AAPARAMS.get(peptide.charAt(sze-1)).RCNS+ // ultimate
					AAPARAMS.get(peptide.charAt(sze-2)).RCN2S; // and penultimate aa

			for (i=2; i<sze-2; i++) { // add weights for aa's in the middle
				tsum3+=AAPARAMS.get(peptide.charAt(i)).RCS;
			}
		} else { // longer peptides use regular retention weights
			tsum3=AAPARAMS.get(peptide.charAt(0)).RC1+ // Sum weights for 1st
					AAPARAMS.get(peptide.charAt(1)).RC2+ // second,
					AAPARAMS.get(peptide.charAt(sze-1)).RCN+ // ultimate
					AAPARAMS.get(peptide.charAt(sze-2)).RCN2; // and penultimate aa

			for (i=2; i<sze-2; i++) { // add weights for aa's in the middle
				tsum3+=AAPARAMS.get(peptide.charAt(i)).RC;
			}
		}
		// _log.debug("Core = "+tsum3);

		// 1- smallness - adjust based on tsum score of peptides shorter than 20
		// aa's.
		tsum3+=smallness(sze, tsum3);
		// _log.debug("smallness = "+tsum3);
		// 2- undigested parts
		tsum3-=undigested(peptide);
		// _log.debug("undigested = "+tsum3);
		// 3- clusterness # NB:weighting of v1 is now done in subrtn.
		tsum3-=clusterness(peptide);
		// _log.debug("clusterness = "+tsum3);
		// 4- proline fix
		tsum3-=proline(peptide);
	...
	private static double undigested(String sq) {
		...
		// --translator's note
		// the perl code does not jibe with the comment above, and will probably
		// need repair
		// possibly dd should start out as 2, not 0; and should loop to xx-2,
		// not xx.

		// Note that negative indices on the perl substr function make
		// substrings offset from right
		// (instead of left) end of string. The perl loop gets negative indices.
		// This may be a
		// a problem.
		...
		for (dd=0; dd<xx; dd++) {
			...
			if (re=='K'||re=='R'||re=='H') {
				op1=op2=op3=op4='\0';
				if (dd-1>=0&&dd-1<=xx)
					op1=sq.charAt(dd-1); // left by 1
				if (dd-2>=0&&dd-2<=xx)
					op2=sq.charAt(dd-2); // left by 2
				if (DUPLICATE_ORIGINAL_CODE) {
					if (dd-1<0&&(-(dd-1))<=xx)
						op1=sq.charAt(xx+(dd-1)+1);
					if (dd-2<0&&(-(dd-2))<=xx)
						op2=sq.charAt(xx+(dd-2)+1);
				}
				if (dd+1>=0&&dd+1<=xx)
					op3=sq.charAt(dd+1); // right by 1
				if (dd+2>=0&&dd+2<=xx)
					op4=sq.charAt(dd+2); // right by 2;

				csum=csum+(UDF31*(AAPARAMS.get(op1).UndKRH+AAPARAMS.get(op3).UndKRH))+(UDF32*(AAPARAMS.get(op2).UndKRH+AAPARAMS.get(op4).UndKRH));
			}
		}
		return csum;
	}
	...
	private static double clusterness(String sq) {
		...
		score=0.0;
		//
		// Translator's note: check on true meaning of the algorithm that
		// defines 'occurs'
		// Should an encoded aa string such as 015101510 match pick "01510" once
		// or twice?
	...
	private static double CalcR(double pH, double PK0, double PK1, int CNTref[]) {
		double cr0=_partial_charge(PK0, pH) // n terminus
		...
		/*
		 * // The following was taken out of the formula for R // - $CNTref->{C}
		 * * _partial_charge( $pH, $PK{C} ) // cys
		 */
		return cr0;
	}
	...
	private static double helicity1(String sq) {
		...
		/*
		 * translator's note: notice lowercase 'z'. This never appears in any
		 * patterns to which this string is compared, and will never match any
		 * helicity patterns.
		 */
	...
	private static double evalH2pattern(String pattern, String testsq, int posn, char etype) {
		...
		for (i=1; i<pattern.length()-2; i=i+3) {
			...
			if (fpart.equals("<-")) {
				iss=1;
				far1=testsqCopy.charAt(i+1);
				far2='\0';
			}
			if (fpart.equals("->")) {
				iss=-1;
				far1='\0';
				far2=testsqCopy.charAt(i+3);
			}
	...
	private static double connector(char acid, char lp, char rp, String ct, char far1, char far2) {
		...
		if (acid=='L'||acid=='W'||acid=='F'||acid=='I') {
			if (((lp=='P'||lp=='G')||(rp=='P'||rp=='G'))&&(!ct.contains("--")))
				mult=0.0;
			if (((far1=='P'||far1=='G')||(far2=='P'||far2=='G'))&&(ct.contains("<-")||ct.contains("->")))
				mult=0.0;
		}
		return mult;
	}
	...
	private static double[] heli2Calc(String sq) {
		// Translator's note: in the original perl and translated C, this
		// function
		// was void and returned values through double pointer arguments. Like
		// this:
		//
		...
		for (i=0; i<pass1.length(); i++) {
			...
			if (m=='1') {
				...
				while (zap<=llim&&subt<2) {
					if (zap<0||zap>=lc.length()) {
						f1='0';
					} else {
						f1=lc.charAt(zap);
					}
					if (zap-1<0||zap-1>=lc.length()) {
						f2='0';
					} else {
						f2=lc.charAt(zap-1);
					}
	...
	public static void main(String argv[]) {
		...
		String[] pep=new String[] { "QGEVVGIVGESGSGK", "GSGSEGVIGVVEGQK", "VIWNCTLQVFQACSELTR", "IRQLQNMNMAQLQAEIAK", "TLESCAQFVQLTCNWIVR", "AIEAQLQAMNMNQLQRIK", "EYNAAPPLQGFGISAPDQVK",
