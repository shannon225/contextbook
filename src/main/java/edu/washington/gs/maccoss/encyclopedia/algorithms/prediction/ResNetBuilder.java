...
public class ResNetBuilder extends ZooModel {
    ...
    public static void identityBlock(ComputationGraphConfiguration.GraphBuilder graph, int[] kernelSize, int[] filters,
    ...
    public static void convBlock(ComputationGraphConfiguration.GraphBuilder graph, int[] kernelSize, int[] filters,
                    String stage, String block, String input) {
    ...
    public ComputationGraphConfiguration.GraphBuilder graphBuilder() {
        int peptideInputSize = 128;  // Example size for peptide one-hot encoding
        int chargeInputSize = 32;    // Example size for charge state one-hot encoding
        int numBClasses = 10;        // Example output size for b ions prediction
        int numYClasses = 10;        // Example output size for y ions prediction
    	...
    	ComputationGraphConfiguration.GraphBuilder graph = new NeuralNetConfiguration.Builder()
    		...
    		graph.addInputs("peptide_input", "charge_input")
    		     .setInputTypes(InputType.feedForward(peptideInputSize), InputType.feedForward(chargeInputSize))
    		     // Process the inputs
    		     .addLayer("peptide_dense", new DenseLayer.Builder().nOut(128).activation(Activation.RELU).build(), "peptide_input")
    		     .addLayer("charge_dense", new DenseLayer.Builder().nOut(32).activation(Activation.RELU).build(), "charge_input")
    		     // Merge inputs
    		     .addVertex("merged_inputs", new MergeVertex(), "peptide_dense", "charge_dense")
    		     // Output layers for b and y ions
    		     .addLayer("b_ions_output", new OutputLayer.Builder(LossFunctions.LossFunction.MCXENT).nOut(numBClasses).activation(Activation.SOFTMAX).build(), "merged_inputs")
    		     .addLayer("y_ions_output", new OutputLayer.Builder(LossFunctions.LossFunction.MCXENT).nOut(numYClasses).activation(Activation.SOFTMAX).build(), "merged_inputs")
    		     .setOutputs("b_ions_output", "y_ions_output");

        convBlock(graph, new int[] {3, 3}, new int[] {64, 64, 256}, "2", "a", new int[] {2, 2}, "merged_inputs");
