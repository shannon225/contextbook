...
//				}
				...
				char aaChar;
				if (aas.size()>0) {
					aaString=aas.get(aas.size()-1);
					aaChar=aaString.charAt(0);
				} else {
					aaString="";
					aaChar=AminoAcidConstants.N_TERM;
				}
				...
				if (aas.size()>0) {
					...
					aas.add((modificationMass>=0?"[+":"[")+modificationMass+"]");
				}
			} else {
				aas.add(Character.toString(ca[i]));
			}
	...
	// does not consider experiment-specific mods
...
//				}
				...
				modificationMass=AminoAcidConstants.getStandardAccurateModificationMass(aaChar, modificationMass);
