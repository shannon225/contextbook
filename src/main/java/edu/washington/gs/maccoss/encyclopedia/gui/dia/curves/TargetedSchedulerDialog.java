...
import edu.washington.gs.maccoss.encyclopedia.algorithms.curve.TargetedScheduler;
...
public class TargetedSchedulerDialog extends JDialog {
	...
	public static void launchTargetedSchedulerDialog(Component root) {
		...
		final JTextArea optionalTargetTextArea = new JTextArea(25, 40);
		optionalTargetTextArea.setFont(new Font("Monospaced", Font.PLAIN, 10));
		JScrollPane optionalTargetScrollPane = new JScrollPane(optionalTargetTextArea); 
		optionalTargetScrollPane.setBorder(BorderFactory.createTitledBorder("Optional target accessions (requires exact matches):"));
		optionalTargetTextArea.setEditable(true);
		
		final JTextArea includeTextArea = new JTextArea(25, 40);
		includeTextArea.setFont(new Font("Monospaced", Font.PLAIN, 10));
		JScrollPane includeScrollPane = new JScrollPane(includeTextArea); 
		includeScrollPane.setBorder(BorderFactory.createTitledBorder("Manually vetted peptides (preferred targets):"));
		includeTextArea.setEditable(true);
		...
		options.add(optionalTargetScrollPane);
		options.add(includeScrollPane);
		...
		okButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				final HashSet<String> targets = getTokens(targetTextArea.getText());
				final HashSet<String> optionalTargets = getTokens(optionalTargetTextArea.getText());
				final HashSet<String> includedPeptides = getTokens(includeTextArea.getText());
				final HashSet<String> eliminatedPeptides = getTokens(eliminateTextArea.getText());
				...
				if (saveDir==null) {
					...
					DilutionCurveParameters fittingParams=new DilutionCurveParameters(((Number)maximumPeptidesPerProteinSpinner.getValue()).intValue(), 
							...
							((Number)mzOffsetSpinner.getValue()).floatValue(), targets.toArray(new String[0]), eliminatedPeptides.toArray(new String[0]));

					try {
						TargetedScheduler.generateAssay(params, saveDir, libraryFile, exampleSearchFile, optionalTargets, targets, 
								includedPeptides, eliminatedPeptides, fittingParams);
					} catch (Exception ex) {
	...
	private static HashSet<String> getTokens(final String text) {
