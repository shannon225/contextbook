...
public class SpectrumPeakFilter {
	...
	public static Stripe filterPeaks(Stripe stripe) {
		...
		return new Stripe(stripe.getSpectrumName(), stripe.getPrecursorName(), stripe.getSpectrumIndex(), stripe.getScanStartTime(), stripe.getIsolationWindowLower(), stripe.getIsolationWindowUpper(),
				arrays.x, arrays.y, stripe.getCharge());
	}

	public static LibraryEntry filterPeaks(LibraryEntry entry) {
		...
		return new LibraryEntry(entry.getSource(), entry.getAccessions(), entry.getSpectrumIndex(), entry.getPrecursorMZ(), entry.getPrecursorCharge(), entry.getPeptideModSeq(), entry.getCopies(),
				entry.getRetentionTime(), entry.getScore(), arrays.x, arrays.y, arrays.z);
	}
