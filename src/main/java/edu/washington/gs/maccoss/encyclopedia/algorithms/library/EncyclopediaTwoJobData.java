...
public class EncyclopediaTwoJobData extends EncyclopediaJobData implements LibrarySearchJobData, XMLObject {
	public static final String LOG_FILE_SUFFIX=".log";
	public static final String DECOY_PROTEIN_FILE_SUFFIX=".encyclopedia2.protein_decoy.txt";
	public static final String OUTPUT_PROTEIN_FILE_SUFFIX=".encyclopedia2.protein.txt";
	public static final String DECOY_FILE_SUFFIX=".encyclopedia2.decoy.txt";
	public static final String OUTPUT_FILE_SUFFIX=".encyclopedia2.txt";
	public static final String FEATURE_FILE_SUFFIX=".features.txt";

	private final LibraryInterface prealignmentLibrary;

	public EncyclopediaTwoJobData(File diaFile, File fastaFile, LibraryInterface prealignmentLibrary, LibraryInterface library, LibraryScoringFactory taskFactory) {
		this(diaFile, null, getPercolatorExecutionData(diaFile, fastaFile, taskFactory.getParameters()), taskFactory.getParameters(), ProgramType.getGlobalVersion().toString(), prealignmentLibrary, library, taskFactory);
	}

	public EncyclopediaTwoJobData(File diaFile, File fastaFile, LibraryInterface prealignmentLibrary, LibraryInterface library, File outputFile, LibraryScoringFactory taskFactory) {
		this(diaFile, null, getPercolatorExecutionData(outputFile, fastaFile, taskFactory.getParameters()), taskFactory.getParameters(), ProgramType.getGlobalVersion().toString(), prealignmentLibrary, library, taskFactory);
	}

	public EncyclopediaTwoJobData(File diaFile, PercolatorExecutionData percolatorFiles, SearchParameters parameters, String version, LibraryInterface prealignmentLibrary, LibraryInterface library, LibraryScoringFactory taskFactory) {
		this(diaFile, null, percolatorFiles, parameters, version, prealignmentLibrary, library, taskFactory);
	}

	public EncyclopediaTwoJobData(File diaFile, StripeFileInterface diaFileReader, PercolatorExecutionData percolatorFiles, SearchParameters parameters, String version, LibraryInterface prealignmentLibrary, LibraryInterface library, LibraryScoringFactory taskFactory) {
		super(diaFile, diaFileReader, percolatorFiles, parameters, version, library, taskFactory);

		this.prealignmentLibrary=prealignmentLibrary;
	}
	...
	public void writeToXML(Document doc, Element parentElement) {
		...
		if (getPrealignmentLibrary() instanceof LibraryFile) {
			XMLUtils.writeTag(doc, rootElement, "prealignmentLibrary", ((LibraryFile) getPrealignmentLibrary()).getFile().getAbsolutePath());
		}
	...
	public static EncyclopediaTwoJobData readFromXML(Document doc, Element rootElement) {
		if (!rootElement.getTagName().equals(EncyclopediaTwoJobData.class.getSimpleName())) {
			throw new EncyclopediaException("Unexpected XML parsing element, found ["+rootElement.getTagName()+"] when expecting ["+EncyclopediaTwoJobData.class.getSimpleName()+"]");
		}
		File diaFile=null;
		File prealignmentLibrary=null;
		File library=null;
		String version=null;
		...
		for (int i = 0; i < nodes.getLength(); i++) {
            ...
            if (node.getNodeType() == Node.ELEMENT_NODE) {
                ...
                } else if ("prealignmentLibrary".equals(element.getTagName())) {
                	prealignmentLibrary=new File(element.getTextContent());
		...
		if (prealignmentLibrary==null) throw new EncyclopediaException("Found null prealignmentLibrary in "+rootElement.getTagName());
		...
		LibraryInterface prealignmentLibraryObject=BlibToLibraryConverter.getFile(prealignmentLibrary, percolatorData.getFastaFile(), readParams);
		...
		return new EncyclopediaTwoJobData(diaFile,  percolatorData, readParams,  version, prealignmentLibraryObject, libraryObject, factory);
	}

	@Override
	public SearchJobData updateQuantFile(File f) {
		return new EncyclopediaTwoJobData(f, getPercolatorFiles(), getParameters(), getVersion(), getPrealignmentLibrary(), getLibrary(), getTaskFactory());
	}
	...
	protected static PercolatorExecutionData getPercolatorExecutionData(File referenceFileLocation, File fastaFile, SearchParameters parameters) {
		return new PercolatorExecutionData(new File(getPrefixFromOutput(referenceFileLocation) + FEATURE_FILE_SUFFIX), fastaFile,
				...
				new File(getPrefixFromOutput(referenceFileLocation) + OUTPUT_PROTEIN_FILE_SUFFIX), new File(getPrefixFromOutput(referenceFileLocation) + DECOY_PROTEIN_FILE_SUFFIX), parameters, false);
	}
	...
	public EncyclopediaTwoJobData updateTaskFactory(LibraryScoringFactory taskFactory) {
		return new EncyclopediaTwoJobData(getDiaFile(), diaFileReader, getPercolatorFiles(), getParameters(), getVersion(), getPrealignmentLibrary(), getLibrary(), taskFactory);
	}
	
	public LibraryInterface getPrealignmentLibrary() {
		return prealignmentLibrary;
	}
	 ...
	 */
	public static EncyclopediaTwoJobData getDummyFor(File diaFile, File fastaFile, LibraryInterface prealignmentLibrary, LibraryInterface library, LibraryScoringFactory factory) {
		return new DummyEncyclopediaJobData(diaFile, fastaFile, prealignmentLibrary, library, factory);
	}
	 ...
	 * otherwise throws on attempts to read the .DIA's contents.
	 */
	private static class DummyEncyclopediaJobData extends EncyclopediaTwoJobData {
		private String originalFileName = null;
		private Float tic = null;

		private DummyEncyclopediaJobData(File diaFile, File fastaFile, LibraryInterface prealignmentLibrary, LibraryInterface library, LibraryScoringFactory factory) {
			super(diaFile, fastaFile, prealignmentLibrary, library, factory);
