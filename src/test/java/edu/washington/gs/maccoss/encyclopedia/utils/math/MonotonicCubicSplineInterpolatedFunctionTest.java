...
public class MonotonicCubicSplineInterpolatedFunctionTest extends TestCase {
	public void testInterpolation() {
		...
		double[] y = {
			    ...
			    0.869, 0.901, 0.928, 0.952, 0.971, 0.985, 0.995, 1.000, 0.999, 0.994
			};
		...
		float[] expectedY = {
			    ...
			    0.998f, 0.998f, 0.998f // never quite gets to 1 because of the downward trend back!
			};
		
		MonotonicCubicSplineInterpolatedFunction function=new MonotonicCubicSplineInterpolatedFunction(points, true);
		for (int i = 0; i < expectedX.length; i++) {
			assertEquals("Error with X="+expectedX[i], expectedY[i], function.getYValue(expectedX[i]), 0.001f);
		}
	}
}
