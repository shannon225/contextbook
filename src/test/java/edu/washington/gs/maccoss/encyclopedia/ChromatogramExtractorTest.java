...
public class ChromatogramExtractorTest {
	public static void main(String[] args) throws Exception {
		...
		files=new File[] {
				...
				new File("/Users/searleb/Documents/projects/encyclopedia/mzml/freezer/121115_bcs_hela_24mz_400_1000_rt_7D_3.mzML")
		};
		...
		File bigPercolatorFile=new File(dir, "concatenated_results.txt");
		ArrayList<ScoredObject<String>> globalPassingPeptides=PercolatorReader.getPassingPeptidesFromTSV(bigPercolatorFile, extractionParameters.getPercolatorThreshold());
		
		for (File file : files) {
			if (file.getName().endsWith("mzML")) {
				long currentTime=System.currentTimeMillis();
				SearchJobData job=getData(extractionParameters, file);
				ChromatogramExtractor.parse(globalPassingPeptides, job, libraryTemplate);
